...
const saveAccounts = () => localStorage.setItem('delmar-accounts', JSON.stringify(accounts));
...
createAccountForm.addEventListener('submit', (event) => { event.preventDefault(); const discordId = authDiscordId.value.trim(); const discordName = authDiscordName.value.trim(); const displayName = authDisplayName.value.trim(); const discordIdKey = normalizeIdentity(discordId); if (accounts.some((account) => account.discordIdKey === discordIdKey)) { authMessage.textContent = 'An account with that email already exists. Log in instead.'; return; } const account = { id: `account-${Date.now()}`, discordId, discordName, discordIdKey, discordNameKey: normalizeIdentity(discordName), name: displayName, roles: ['Buyer'], createdAt: new Date().toISOString() }; accounts.push(account); saveAccounts(); rememberAccount(account); localStorage.setItem('delmar-profile', JSON.stringify({ name: displayName, theme: 'coast', avatar: '' })); profileName.value = displayName; profileRole.textContent = roleSummary(account); profileSummary.textContent = `${displayName} · ${roleSummary(account)}`; renderAvatar('', displayName); renderAdminPanel(); hideAuthGate(); });
...
const setAdminOpen = (isOpen) => { const canOpenAdmin = hasRole(currentAccount(), 'Admin'); const shouldOpen = isOpen && canOpenAdmin; adminDrawer.hidden = !shouldOpen; if (shouldOpen) { profilePanel.hidden = true; profileButton.setAttribute('aria-expanded', 'false'); } profileBackdrop.hidden = !shouldOpen; adminButton.setAttribute('aria-expanded', String(shouldOpen)); if (shouldOpen) accountList.querySelector('select')?.focus(); };
...
accountList?.addEventListener('change', (event) => { if (!event.target.matches('[data-account-id][data-role]')) return; const account = accounts.find((item) => item.id === event.target.dataset.accountId); if (!account) return; account.roles = accountRoles(account).filter((role) => role.toLowerCase() !== event.target.dataset.role.toLowerCase()); if (event.target.checked) account.roles.push(event.target.dataset.role); saveAccounts(); profileRole.textContent = roleSummary(currentAccount()); profileSummary.textContent = profileName.value ? `${profileName.value} · ${roleSummary(currentAccount())}` : roleSummary(currentAccount()); renderAdminPanel(); toast.textContent = `${account.name} roles updated`; toast.classList.add('show'); window.setTimeout(() => toast.classList.remove('show'), 2200); });
accountList?.addEventListener('click', (event) => { const deleteButton = event.target.closest('[data-delete-account]'); if (!deleteButton) return; const account = accounts.find((item) => item.id === deleteButton.dataset.deleteAccount); if (!account || !window.confirm(`Delete ${account.name}'s profile?`)) return; accounts = accounts.filter((item) => item.id !== account.id); saveAccounts(); renderAdminPanel(); toast.textContent = `${account.name}'s profile deleted`; toast.classList.add('show'); window.setTimeout(() => toast.classList.remove('show'), 2200); });
accountSearch?.addEventListener('input', renderAdminPanel);
accountForm?.addEventListener('submit', (event) => { event.preventDefault(); const name = newAccount.value.trim(); if (!name) return; const id = `${name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${Date.now()}`; accounts.push({ id, name, discordId: id, discordName: name, discordIdKey: normalizeIdentity(id), discordNameKey: normalizeIdentity(name), roles: ['Buyer'], createdAt: new Date().toISOString() }); saveAccounts(); newAccount.value = ''; renderAdminPanel(); setAdminOpen(true); });

window.addEventListener('storage', (event) => { if (event.key !== 'delmar-accounts' || !event.newValue) return; accounts = JSON.parse(event.newValue); renderAdminPanel(); });
